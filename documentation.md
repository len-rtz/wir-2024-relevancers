...
It is trained using ms-marco query pairs data and fine tuned weith a Reinforcement Learning framework using a policy gradient approach to fine-tune the policy (sequence-to-sequence model).
...
Our senond test system uses a sentence-transformer model, primarily designed for semantic similarity tasks and embedding generation
